...
class ComplianceBriefRequest(BaseModel):
...
class OpsRiskResponse(BaseModel):
    status: str
    risk_score: float
    risk_level: str
    controls: List[str]
...
class GeneticIPEngine:
    ...
    def compliance_explainer(
        self,
        sequence: Optional[GenomeSequenceDB] = None,
        license_record: Optional[LicenseDB] = None,
        language: str = "en",
    ) -> Dict[str, Any]:
        """Create a plain-language compliance explainer for a sequence or license."""
        target_name = sequence.species_name if sequence else (license_record.license_id if license_record else "unknown")
        source_nation = sequence.source_nation if sequence else "Unknown"
        licensee_nation = license_record.licensee_nation if license_record else "Unknown"
        license_type = license_record.license_type if license_record else "research"

        compliance = {
            "source_nation": source_nation,
            "licensee_nation": licensee_nation,
            "license_type": license_type,
            "nagoya_sensitive": bool(sequence and sequence.nagoya_compliant),
            "cross_border": source_nation != licensee_nation,
            "royalty_rate_pct": self.LICENSE_ROYALTY_RATES.get(license_type, 3.0),
        }

        translations = {
            "en": {
                "overview": f"{target_name} is eligible for compliance review with clear Nagoya benefit-sharing terms.",
                "actions": [
                    "Confirm prior informed consent before transfer or use.",
                    "Attach mutually agreed terms for scope, royalties, and reporting.",
                    "Keep the provenance record and license audit visible to operators.",
                ],
                "risk_prefix": "Risk",
            },
            "ar": {
                "overview": f"{target_name} جاهز لمراجعة الامتثال مع شروط واضحة لتقاسم المنافع.",
                "actions": [
                    "تأكيد الموافقة المسبقة قبل النقل أو الاستخدام.",
                    "إرفاق شروط متفق عليها للنطاق والرسوم والتقارير.",
                    "إبقاء سجل المصدر والتدقيق متاحين للمشغلين.",
                ],
                "risk_prefix": "مخاطر",
            },
            "fr": {
                "overview": f"{target_name} est prêt pour une revue de conformité avec des règles claires de partage des avantages.",
                "actions": [
                    "Confirmer le consentement préalable avant le transfert ou l'usage.",
                    "Joindre des conditions convenues pour la portée, les redevances et le reporting.",
                    "Conserver la provenance et la piste d'audit visibles.",
                ],
                "risk_prefix": "Risque",
            },
            "es": {
                "overview": f"{target_name} está listo para revisión de cumplimiento con reglas claras de reparto de beneficios.",
                "actions": [
                    "Confirmar consentimiento previo antes de transferir o usar.",
                    "Adjuntar términos acordados sobre alcance, regalías e informes.",
                    "Mantener visible el origen y la pista de auditoría.",
                ],
                "risk_prefix": "Riesgo",
            },
        }
        selected = translations.get(language.lower(), translations["en"])

        highlights = [
            f"Source nation: {source_nation}",
            f"Licensee nation: {licensee_nation}",
            f"License type: {license_type}",
            f"Royalty rate: {compliance['royalty_rate_pct']}%",
        ]
        risks = []
        if compliance["cross_border"]:
            risks.append(f"{selected['risk_prefix']}: cross-border access needs extra documentation.")
        if license_type in ("commercial", "exclusive"):
            risks.append(f"{selected['risk_prefix']}: commercial usage increases benefit-sharing and reporting obligations.")
        if not compliance["nagoya_sensitive"]:
            risks.append(f"{selected['risk_prefix']}: verify provenance completeness before publication.")

        return {
            "language": language.lower(),
            "overview": selected["overview"],
            "highlights": highlights,
            "risks": risks or [f"{selected['risk_prefix']}: no immediate compliance blockers detected."],
            "actions": selected["actions"],
        }

    def workflow_templates(self) -> List[Dict[str, Any]]:
        """Return n8n-ready workflow templates for licensing and royalty automation."""
        return [
            {
                "workflow_id": "license-onboarding",
                "name": "License onboarding and compliance review",
                "trigger": "sequence_registered",
                "status": "active",
                "steps": [
                    "Fetch sequence metadata",
                    "Run Nagoya compliance explainer",
                    "Queue legal review if cross-border",
                    "Create license draft",
                ],
                "n8n_payload": {
                    "name": "License Onboarding",
                    "nodes": [
                        {"id": "fetch_sequence", "type": "httpRequest"},
                        {"id": "check_compliance", "type": "function"},
                        {"id": "review_gate", "type": "if"},
                        {"id": "draft_license", "type": "set"},
                    ],
                },
            },
            {
                "workflow_id": "royalty-reconciliation",
                "name": "Royalty reconciliation and reminder loop",
                "trigger": "royalty_recorded",
                "status": "active",
                "steps": [
                    "Validate payment period",
                    "Compute royalty breakdown",
                    "Notify source nation and internal ops",
                    "Log audit trail and dashboard refresh",
                ],
                "n8n_payload": {
                    "name": "Royalty Reconciliation",
                    "nodes": [
                        {"id": "fetch_payment", "type": "httpRequest"},
                        {"id": "compute_breakdown", "type": "function"},
                        {"id": "notify", "type": "email"},
                        {"id": "refresh_dashboard", "type": "set"},
                    ],
                },
            },
        ]

    def ops_risk_summary(self, db: Session) -> Dict[str, Any]:
        """Summarize lightweight operations and marketplace risk."""
        active_sequences = db.query(GenomeSequenceDB).filter(GenomeSequenceDB.is_active == True).count()
        active_licenses = db.query(LicenseDB).filter(LicenseDB.status == "active").count()
        overdue_royalties = db.query(RoyaltyPaymentDB).filter(RoyaltyPaymentDB.status != "recorded").count()
        compliance_flags = db.query(ComplianceAuditDB).filter(ComplianceAuditDB.result != "pass").count()

        risk_score = min(
            100.0,
            round(active_sequences * 1.5 + active_licenses * 2.0 + overdue_royalties * 12.0 + compliance_flags * 15.0, 2),
        )
        risk_level = "low" if risk_score < 20 else "moderate" if risk_score < 50 else "elevated" if risk_score < 80 else "critical"
        controls = [
            "RBAC and audit logging enabled",
            "Nagoya compliance checks on license issuance",
            "Royalty reconciliation tracked in the ledger",
            "Workflow templates available for automation handoff",
        ]
        watch_items = []
        if overdue_royalties:
            watch_items.append("Unrecorded royalty events")
        if compliance_flags:
            watch_items.append("Non-pass compliance audits")
        if active_sequences == 0:
            watch_items.append("No active listings to monetize")

        return {
            "status": "healthy" if risk_level in ("low", "moderate") else "watch",
            "risk_score": risk_score,
            "risk_level": risk_level,
            "controls": controls,
            "watch_items": watch_items or ["No immediate watch items detected."],
...
@app.post("/api/v1/compliance/brief", tags=["Compliance"])
def compliance_brief(payload: ComplianceBriefRequest, db: Session = Depends(get_db)):
    """Generate a human-readable compliance explainer for a sequence or license."""
    sequence = None
    license_record = None

    if payload.sequence_id:
        sequence = db.query(GenomeSequenceDB).filter(GenomeSequenceDB.sequence_id == payload.sequence_id).first()
        if not sequence:
            raise HTTPException(status_code=404, detail="Sequence not found")

    if payload.license_id:
        license_record = db.query(LicenseDB).filter(LicenseDB.license_id == payload.license_id).first()
        if not license_record:
            raise HTTPException(status_code=404, detail="License not found")
        if sequence is None:
            sequence = db.query(GenomeSequenceDB).filter(GenomeSequenceDB.sequence_id == license_record.sequence_id).first()

    if sequence is None and license_record is None:
        raise HTTPException(status_code=400, detail="Provide sequence_id or license_id")

    return genetic_engine.compliance_explainer(sequence=sequence, license_record=license_record, language=payload.language)


@app.get("/api/v1/compliance/summary", response_model=ComplianceSummaryResponse, tags=["Compliance"])
def compliance_summary(language: str = Query(default="en", min_length=2, max_length=8), db: Session = Depends(get_db)):
    """Return a multilingual marketplace compliance summary."""
    base = genetic_engine.ops_risk_summary(db)
    language = language.lower()
    messages = {
        "en": {
            "overview": "Marketplace compliance is centered on Nagoya checks, provenance tracking, and royalty accountability.",
            "highlights": [
                f"{db.query(GenomeSequenceDB).count()} sequences cataloged",
                f"{db.query(LicenseDB).count()} licenses issued",
                f"{db.query(RoyaltyPaymentDB).count()} royalty records tracked",
            ],
            "risks": base["watch_items"],
            "actions": [
                "Review unresolved compliance audits.",
                "Keep license and royalty workflows automated.",
                "Use multilingual summaries for stakeholder reporting.",
            ],
        },
        "ar": {
            "overview": "يركز الامتثال في السوق على فحوصات ناجويا وتتبع المصدر ومساءلة العوائد.",
            "highlights": [
                f"عدد التسلسلات المسجلة: {db.query(GenomeSequenceDB).count()}",
                f"عدد التراخيص: {db.query(LicenseDB).count()}",
                f"عدد سجلات العوائد: {db.query(RoyaltyPaymentDB).count()}",
            ],
            "risks": base["watch_items"],
            "actions": [
                "مراجعة تقارير التدقيق غير المنتهية.",
                "أتمتة مسارات التراخيص والعوائد.",
                "استخدام ملخصات متعددة اللغات للتقارير.",
            ],
        },
        "fr": {
            "overview": "La conformité du marché repose sur les contrôles Nagoya, la traçabilité et la responsabilité des redevances.",
            "highlights": [
                f"{db.query(GenomeSequenceDB).count()} séquences cataloguées",
                f"{db.query(LicenseDB).count()} licences émises",
                f"{db.query(RoyaltyPaymentDB).count()} écritures de redevances",
            ],
            "risks": base["watch_items"],
            "actions": [
                "Examiner les audits de conformité ouverts.",
                "Automatiser les flux de licences et de redevances.",
                "Utiliser des résumés multilingues pour les parties prenantes.",
            ],
        },
        "es": {
            "overview": "El cumplimiento del mercado se centra en controles Nagoya, trazabilidad y responsabilidad de regalías.",
            "highlights": [
                f"{db.query(GenomeSequenceDB).count()} secuencias catalogadas",
                f"{db.query(LicenseDB).count()} licencias emitidas",
                f"{db.query(RoyaltyPaymentDB).count()} registros de regalías",
            ],
            "risks": base["watch_items"],
            "actions": [
                "Revisar auditorías de cumplimiento abiertas.",
                "Automatizar los flujos de licencias y regalías.",
                "Usar resúmenes multilingües para informes ejecutivos.",
            ],
        },
    }
...
@app.get("/api/v1/workflows/templates", response_model=List[WorkflowTemplateResponse], tags=["Workflows"])
def get_workflow_templates():
    """Return n8n-ready workflow templates and their current status."""
    return genetic_engine.workflow_templates()


@app.get("/api/v1/workflows/templates/{workflow_id}", response_model=WorkflowTemplateResponse, tags=["Workflows"])
def get_workflow_template(workflow_id: str):
    """Fetch a specific workflow template by ID."""
    for workflow in genetic_engine.workflow_templates():
        if workflow["workflow_id"] == workflow_id:
            return workflow
    raise HTTPException(status_code=404, detail="Workflow not found")


@app.get("/api/v1/ops/risk-summary", response_model=OpsRiskResponse, tags=["Operations"])
def ops_risk_summary(db: Session = Depends(get_db)):
    """Provide a lightweight operations and risk summary for the marketplace."""
    return genetic_engine.ops_risk_summary(db)
