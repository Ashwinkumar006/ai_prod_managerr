...
class ResearchDigestRequest(BaseModel):
    specimen_code: str
    language: str = Field(default="en", pattern="^(en|ar|fr|es)$")
    ...
    def predict_targets(self, bioactivity: Dict[str, float]) -> List[Dict[str, Any]]:
        ...
        target_map = {
            "anti-inflammatory": [
                ...
                {"target": "NF-kB", "pathway": "Inflammatory signaling"},
                {"target": "TNF-alpha", "pathway": "Cytokine signaling"},
            ],
            "anti-cancer": [
...
RESEARCH_DIGEST_TEMPLATE_ID = "cryptonova.research.digest"
RESEARCH_DIGEST_PROMPT_VERSION = "2026-03-22.1"
MLOPS_MODEL_NAME = "compound-prioritization"
MLOPS_MODEL_VERSION = "0.3.0"
MLOPS_EXPERIMENT_ID = "CN-EXP-20260322"

RESEARCH_LANGUAGE_COPY = {
    "en": {
        "title": "Research Digest",
        "overview_label": "Overview",
        "workflow_label": "Workflow status",
        "next_steps_label": "Next steps",
        "export_ready": "Board-ready brief available",
    },
    "ar": {
        "title": "Mulakhas al-Bahth",
        "overview_label": "Nazra Aamma",
        "workflow_label": "Halat al-Tadfouq",
        "next_steps_label": "Al-Khatawat al-Taliya",
        "export_ready": "Mojaz jahez lil-ard",
    },
    "fr": {
        "title": "Synthese de Recherche",
        "overview_label": "Vue d'ensemble",
        "workflow_label": "Statut du workflow",
        "next_steps_label": "Prochaines etapes",
        "export_ready": "Brief executif pret",
    },
    "es": {
        "title": "Resumen de Investigacion",
        "overview_label": "Resumen general",
        "workflow_label": "Estado del flujo",
        "next_steps_label": "Siguientes pasos",
        "export_ready": "Informe ejecutivo listo",
    },
}


def _language_copy(language: str) -> Dict[str, str]:
    return RESEARCH_LANGUAGE_COPY.get(language, RESEARCH_LANGUAGE_COPY["en"])


def _build_research_workflow(
    specimen: SpecimenDB,
    compounds: List[BiocompoundDB],
    briefs: List[PatentBriefDB],
) -> WorkflowStatusResponse:
    classified = specimen.classification_status != "unclassified"
    compound_status = "completed" if compounds else "in_progress"
    brief_status = "completed" if briefs else ("in_progress" if compounds else "pending")
    completed_steps = sum([1, int(classified), int(bool(compounds)), int(bool(briefs))])
    completion_pct = round((completed_steps / 4) * 100, 2)

    steps = [
        WorkflowStepStatus(
            step_id="specimen_intake",
            name="Specimen intake",
            status="completed",
            owner="product-bioops",
            evidence={"specimen_code": specimen.specimen_code, "created_at": specimen.created_at.isoformat()},
        ),
        WorkflowStepStatus(
            step_id="taxonomic_classification",
            name="Taxonomic classification",
            status="completed" if classified else "pending",
            owner="research-agent",
            evidence={"classification_status": specimen.classification_status, "gbif_match_score": specimen.gbif_match_score},
        ),
        WorkflowStepStatus(
            step_id="compound_discovery",
            name="Compound discovery",
            status=compound_status,
            owner="discovery-engine",
            evidence={"compound_count": len(compounds), "top_value_usd": max((c.estimated_value_usd for c in compounds), default=0)},
        ),
        WorkflowStepStatus(
            step_id="patent_briefing",
            name="Patent briefing",
            status=brief_status,
            owner="ip-agent",
            evidence={"brief_count": len(briefs), "latest_brief": briefs[-1].brief_code if briefs else None},
        ),
    ]

    overall_status = (
        "completed" if completion_pct == 100 else
        "in_progress" if completion_pct >= 50 else
        "early_stage"
    )

    return WorkflowStatusResponse(
        workflow_code=f"WF-{specimen.specimen_code}",
        specimen_code=specimen.specimen_code,
        status=overall_status,
        completion_pct=completion_pct,
        steps=steps,
        export_format="n8n-json",
    )
...
def _build_research_digest(
    ...
    top_compounds = sorted(compounds, key=lambda c: c.estimated_value_usd, reverse=True)[:3]
    top_brief = sorted(briefs, key=lambda b: b.generated_at, reverse=True)[:1]
    workflow = _build_research_workflow(specimen, compounds, briefs)

    next_steps = [
        "Validate the leading therapeutic hypothesis with wet-lab partners.",
        "Advance the highest-value compound into patent review.",
        "Package the story as an executive-ready innovation brief.",
    ]
    if language == "fr":
        next_steps = [
            "Valider l'hypothese therapeutique principale avec les partenaires laboratoire.",
            "Faire avancer le compose le plus prometteur vers la revue brevet.",
            "Preparer une note d'innovation pour la direction.",
        ]
    elif language == "es":
        next_steps = [
            "Validar la hipotesis terapeutica principal con socios de laboratorio.",
            "Llevar el compuesto de mayor valor a revision de patente.",
            "Preparar un informe ejecutivo de innovacion.",
        ]
    elif language == "ar":
        next_steps = [
            "Taakid al-faradiya al-ilajiya al-raisiya ma shuraka al-mukhtabar.",
            "Naql al-murakkab al-aala qima ila murajaat al-baraya.",
            "Iidad mojaz tanfithi lil-ibtikar.",
        ]

    overview = (
        f"{specimen.name} is currently tracked as {specimen.classification_status}. "
        f"The program has generated {len(compounds)} compounds and {len(briefs)} patent briefs."
    )
    if language == "fr":
        overview = (
            f"{specimen.name} est suivi avec le statut {specimen.classification_status}. "
            f"Le programme a genere {len(compounds)} composes et {len(briefs)} briefs brevet."
        )
    elif language == "es":
        overview = (
            f"{specimen.name} se sigue con estado {specimen.classification_status}. "
            f"El programa genero {len(compounds)} compuestos y {len(briefs)} briefs de patente."
        )
    elif language == "ar":
        overview = (
            f"{specimen.name} marsood bihalat {specimen.classification_status}. "
            f"Albarnamaj wallad {len(compounds)} murakkabat wa {len(briefs)} patent briefs."
        )
    ...
    return {
        ...
        "specimen_code": specimen.specimen_code,
        "prompt_metadata": {
            "template_id": RESEARCH_DIGEST_TEMPLATE_ID,
            "prompt_version": RESEARCH_DIGEST_PROMPT_VERSION,
            "generated_at": datetime.utcnow().isoformat(),
        },
        "sections": {
            "overview_label": copy["overview_label"],
            "overview": overview,
            "workflow_label": copy["workflow_label"],
            "workflow_status": workflow.model_dump(),
            "top_compounds": [
                {
                    "compound_code": c.compound_code,
                    "name": c.name,
                    "drug_likeness_score": c.drug_likeness_score,
                    "estimated_value_usd": c.estimated_value_usd,
                }
                for c in top_compounds
            ],
            "latest_patent_brief": (
                {
                    "brief_code": top_brief[0].brief_code,
                    "title": top_brief[0].title,
                    "patentability_score": top_brief[0].patentability_score,
                }
                if top_brief else None
            ),
            "next_steps_label": copy["next_steps_label"],
            "next_steps": next_steps,
        },
        "publishing_ready": {
            "status": copy["export_ready"],
            "formats": ["board-brief", "product-narrative", "executive-email"],
        },
    }


def _build_mlops_summary(db: Session) -> MLOpsSummaryResponse:
    specimen_count = db.query(SpecimenDB).count()
    compound_count = db.query(BiocompoundDB).count()
    brief_count = db.query(PatentBriefDB).count()
    avg_dl = db.query(func.avg(BiocompoundDB.drug_likeness_score)).scalar() or 0
    high_value = db.query(BiocompoundDB).filter(BiocompoundDB.estimated_value_usd >= 100_000_000).count()

    recent_runs = [
        {
            "run_id": "RUN-CLASSIFY-001",
            "stage": "classification",
            "status": "completed",
            "artifacts": {"specimens_processed": specimen_count},
        },
        {
            "run_id": "RUN-DISCOVERY-001",
            "stage": "compound_discovery",
            "status": "completed" if compound_count else "pending",
            "artifacts": {"compounds_generated": compound_count},
        },
        {
            "run_id": "RUN-IP-001",
            "stage": "patent_briefing",
            "status": "completed" if brief_count else "in_progress",
            "artifacts": {"briefs_generated": brief_count},
        },
    ]

    return MLOpsSummaryResponse(
        model_name=MLOPS_MODEL_NAME,
        model_version=MLOPS_MODEL_VERSION,
        prompt_version=RESEARCH_DIGEST_PROMPT_VERSION,
        experiment_id=MLOPS_EXPERIMENT_ID,
        active_status="healthy",
        metrics={
            "specimens_registered": specimen_count,
            "compounds_generated": compound_count,
            "patent_briefs_generated": brief_count,
            "average_drug_likeness": round(float(avg_dl), 2),
            "high_value_compounds": high_value,
        },
...
print("P3 main: before enterprise_core import")
import enterprise_core
app.include_router(enterprise_core.router)
print("P3 main: before ghost_extensions import")
import ghost_extensions
app.include_router(ghost_extensions.router)
print("P3 main: after router imports")
...
@app.post("/api/v1/research/digest", tags=["Research"])
def generate_research_digest(payload: ResearchDigestRequest, db: Session = Depends(get_db)):
    """Generate a multilingual research digest from specimen, compound, and patent data."""
    specimen = db.query(SpecimenDB).filter(SpecimenDB.specimen_code == payload.specimen_code).first()
    if not specimen:
        raise HTTPException(status_code=404, detail="Specimen not found")

    compounds = db.query(BiocompoundDB).filter(BiocompoundDB.specimen_id == specimen.id).all()
    briefs = db.query(PatentBriefDB).filter(PatentBriefDB.compound_id.in_([c.id for c in compounds])).all() if compounds else []
    return _build_research_digest(specimen, compounds, briefs, payload.language)


@app.get("/api/v1/specimens/{specimen_code}/workflow-status", response_model=WorkflowStatusResponse, tags=["Research"])
def get_workflow_status(specimen_code: str, db: Session = Depends(get_db)):
    """Return an agentic workflow/export status summary for the discovery pipeline."""
    specimen = db.query(SpecimenDB).filter(SpecimenDB.specimen_code == specimen_code).first()
    if not specimen:
        raise HTTPException(status_code=404, detail="Specimen not found")

    compounds = db.query(BiocompoundDB).filter(BiocompoundDB.specimen_id == specimen.id).all()
    briefs = db.query(PatentBriefDB).filter(PatentBriefDB.compound_id.in_([c.id for c in compounds])).all() if compounds else []
    return _build_research_workflow(specimen, compounds, briefs)


@app.get("/api/v1/mlops/summary", response_model=MLOpsSummaryResponse, tags=["Operations"])
def get_mlops_summary(db: Session = Depends(get_db)):
    """Expose a lightweight MLOps-style summary for portfolio demos and governance."""
